...
function testExpression(e1,e2,e3,e4,e5) {
    ...
    gr.addQuery('sys_id',op[e1],'E1');

    // this is an anchor expression needed to chain the addOrConditions against it.
    // See article
    var x = gr.addQuery('sys_id','=','FALSE'); 

    x.addOrCondition('sys_id',op[e2],'E2').addCondition('sys_id',op[e3],'E3');

    x.addOrCondition('sys_id',op[e4],'E4').addCondition('sys_id',op[e5],'E5');
